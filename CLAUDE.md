...
2. **Intake** — background/experience details, gets assigned a sequential anonymous ID (`SME-1`, `SME-2`, ...)
3. **Video Demo** — a short walkthrough before discussion
4. **Interview** — five open-ended discussion questions (Q2-Q6), free-text notes
5. **Category Validation Exercise** — the centerpiece. Six real findings from a code-review AI tool are shown one at a time, code on the left and the question on the right (side-by-side above 600px, stacked below it). The participant must guess which of 4 categories (Code Quality / Bugs / Optimization / Readability) a finding belongs to **before** the AI's actual answer is revealed, then state agreement.
6. **Hands-on Prototype** — a researcher-supervised, fixed `MySejahteraAlpha.dart` task in the embedded Glance web build: run a review, open feedback, apply one suggested fix.
7. **SUS** — the standard 10-item System Usability Scale, locked until all four hands-on milestones are saved.
8. **Done**
...
```
index.html            Page structure — Cover + Consent + Intake/Demo/Interview/CVE/Hands-on/SUS/Done sections, toggled via .active class
...
**Four Supabase SQL files matter for the live project:** `supabase/migration.sql` (full schema, for a fresh install), `supabase/002_add_explanation.sql` (incremental — adds the `explanation` column + updates `submit_guess`), `supabase/003_add_hands_on_task.sql` (incremental — creates the milestone-only hands-on record and RPC), and `supabase/004_add_participant_consent.sql` (incremental — creates recorded consent and the response-protection triggers). Confirm the three incremental scripts have actually been executed in the SQL Editor before assuming their features work live.
...
- **Exception for the study cover:** `section-cover` appears before Intake to explain the thesis context, the application, and the session. It is not part of the response-collection flow, so it deliberately has no `STEP_ORDER` or stepper entry; `showSection()` hides the stepper while it is visible. The cover stores no data and its only action moves into Intake. Do not place any participant-specific or CVE-reveal information on this page.
