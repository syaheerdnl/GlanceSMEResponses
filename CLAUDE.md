...
researcher.js          Magic-link sign-in, authenticated dashboard rendering, and CSV export logic.
...
**Private researcher results (13 August 2026, local until deployed):** `researcher.html` is a separate, non-participant dashboard reachable from the study footer. It emails a Supabase Auth magic link to the one approved address, `muhammadsyaheerdaniel@gmail.com`. `researcher.js` stores only the short-lived access token in `sessionStorage`, removes it from the callback URL, and calls `researcher_dashboard`. Its magic-link callback is explicitly fixed to `https://syaheerdnl.github.io/GlanceSMEResponses/researcher.html`, so opening a local `file://` copy for testing cannot make Supabase fall back to an invalid local URL. That RPC is granted to `authenticated`, not `anon`, then checks the signed JWT email against the locked-down `researcher_access` table. It returns participant overview data, including names only inside the protected dashboard and participant export, plus CSV export data filterable as All, Full SME, or SUS-only. The Participants CSV includes SUS-only `language_familiarity`. It never sends results to a browser PIN or exposes direct table reads. Before deploying this frontend, run the current 007 after 003, 004 and 005; if older 007 was already applied, run 008 afterwards. Then configure Supabase Authentication URL Configuration with Site URL `https://syaheerdnl.github.io/GlanceSMEResponses/` and Redirect URL `https://syaheerdnl.github.io/GlanceSMEResponses/researcher.html`; Email provider and a template using `{{ .ConfirmationURL }}` must remain enabled.
...
- Don't replace the researcher magic-link check with a browser PIN or put a researcher access token in source code. Results must remain protected by the server-side signed-email allowlist.
