...
1. **Consent** — participant reads the study information and explicitly agrees before an anonymous ID or any response is created. The accepted form version and first timestamp are recorded later against that ID.
...
The **full SME route** then follows: Intake (background and sequential `SME-N` ID) → Video Demo → Interview (Q2-Q6) → Category Validation Exercise → Hands-on Prototype → SUS → Done.

The **SUS-only route** receives the same sequential `SME-N` ID without SME background fields, then follows: Video Demo → Hands-on Prototype → SUS → Done. It never displays Intake, Interview, or the Category Validation Exercise.
...
| `assignSusOnlyId` | `assign_sus_only_id` | none | `id` (e.g. `"SME-4"`) | Inserts only an anonymous `intake` row with `study_path = 'sus_only'`; no code, name, Firebase identity, or application data is sent |
...
### Schema (8 tables, `supabase/migration.sql`)

- **intake** — `participant_code` (generated, e.g. `SME-3`), timestamp, `study_path` (`full_sme` or `sus_only`), and, only for full-SME records, years of experience, platforms/languages, role
...
**Six Supabase SQL files matter for the live project:** `supabase/migration.sql` (full schema, for a fresh install), `supabase/002_add_explanation.sql` (incremental, adds the `explanation` column and updates `submit_guess`), `supabase/003_add_hands_on_task.sql` (incremental, creates the milestone-only hands-on record and RPC), `supabase/004_add_participant_consent.sql` (incremental, creates recorded consent and the response-protection triggers), `supabase/005_add_study_path.sql` (incremental, marks full-SME/SUS-only routes and adds `assign_sus_only_id`), and `supabase/006_add_researcher_dashboard.sql` (incremental, creates the RLS-locked allowlist and authenticated results RPC). Confirm all five incremental scripts have actually been executed in the SQL Editor before assuming their features work live.
