...
- Visual style: high-contrast editorial research-instrument treatment. Playfair Display is reserved for main explanatory headings; Space Grotesk is used for questionnaire copy, labels, controls, and supporting text. Use crisp 1px dark borders, squared corners of at most 3px, no gradients, and no soft shadows. Selection and focus states must be immediate and solid, deep green for selection and crimson for focus/accent, not opacity-fade effects. The SUS section is deliberately a 10-row, alternating-white/light-grey grid with five large custom radio tap targets per row; the native inputs remain underneath for semantics and keyboard use. Keep minimal iconography (none currently; text and colour do the work) rather than introducing a competing visual language.
...
- Don't add analytics or third-party script tags — this is a small academic-research instrument, not a product; minimize external dependencies and privacy surface area. **Exception (explicitly requested 13 August 2026):** `index.html` loads Google Fonts' Playfair Display and Space Grotesk stylesheet solely for the editorial visual system. Do not add other external assets or scripts without user approval. This includes not adding the `@supabase/supabase-js` SDK — plain `fetch()` against Supabase's REST/RPC endpoints is the deliberate choice, see "File map" above.
