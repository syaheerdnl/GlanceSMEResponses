...
**Recorded consent implementation (13 August 2026):** local code adds a Consent page between Cover and Background. The participant cannot progress without ticking the explicit agreement, and `save_consent` records only `participant_id`, `accepted=true`, `sme-web-consent-v3`, and the first consent timestamp. Consent v3 explains that names are held separately and that SUS-only participants record Dart/Kotlin familiarity rather than a professional profile. Migration 007 includes v3 for a fresh upgrade path; migration 008 upgrades older 007 installations while still accepting v1/v2 for a disrupted pre-change session. The consent migration also installs database triggers that reject all interview, CVE, hands-on, and SUS writes without recorded consent. The wording is a practical web form based on the current study scope, not a substitute for any faculty ethics-approved participant-information sheet; check it with the supervisor before live collection.
...
**Eight Supabase SQL files matter for the live project:** `supabase/migration.sql` (full schema, for a fresh install), `supabase/002_add_explanation.sql` (incremental, adds the `explanation` column and updates `submit_guess`), `supabase/003_add_hands_on_task.sql` (incremental, creates the milestone-only hands-on record and RPC), `supabase/004_add_participant_consent.sql` (incremental, creates recorded consent and the response-protection triggers), `supabase/005_add_study_path.sql` (incremental, marks full-SME/SUS-only routes), `supabase/006_add_researcher_dashboard.sql` (the original dashboard migration), `supabase/007_add_shared_background_and_identity.sql` (protected names, current full-SME/SUS-only background fields, consent v3, and enhanced dashboard RPC), and `supabase/008_make_sus_background_route_specific.sql` (upgrade for the original 007 implementation). For an existing project run 003, 004, 005, then the current 007. If original 007 was already run, execute 008 afterwards. If 006 was already run, leave it in place.
