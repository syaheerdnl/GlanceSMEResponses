...
The **SUS-only route** completes the same Background step and receives the same sequential `SME-N` ID, then follows: Video Demo → Hands-on Prototype → SUS → Done. It skips only Interview and the Category Validation Exercise.
...
| `assignSusOnlyId` | `assign_sus_only_id` | `p_years_experience`, `p_platforms`, `p_role`, `p_participant_name` | `id` (e.g. `"SME-4"`) | Inserts the same background and separately protected name with `study_path = 'sus_only'`; no browser code, Firebase identity, or application data is sent |
...
`participantName`, `yearsExperience`, `platforms`, `agreement`, `correctCategory`, and `couldAlsoBe` are all sent as **plain strings**, even though the UI uses richer controls (slider, chips, toggle switch, ranked checkboxes) to produce them. The name is the one exception to the normal background record: assignment RPCs insert it into `participant_identity`, not `intake`. If you add a new field that needs its own column, add it to the relevant table in `supabase/migration.sql` **and** update the corresponding function's `insert`/`update` in the same change, or you get silent data loss. Unlike the old Sheet-based setup, adding a column to an existing table with data in it is a normal `alter table ... add column ...` in the SQL Editor, with no retrofit caveat.

### Schema (9 tables, `supabase/migration.sql`)

- **intake** — `participant_code` (generated, e.g. `SME-3`), timestamp, `study_path` (`full_sme` or `sus_only`), and shared years of experience, platforms/languages, role
...
**Seven Supabase SQL files matter for the live project:** `supabase/migration.sql` (full schema, for a fresh install), `supabase/002_add_explanation.sql` (incremental, adds the `explanation` column and updates `submit_guess`), `supabase/003_add_hands_on_task.sql` (incremental, creates the milestone-only hands-on record and RPC), `supabase/004_add_participant_consent.sql` (incremental, creates recorded consent and the response-protection triggers), `supabase/005_add_study_path.sql` (incremental, marks full-SME/SUS-only routes and adds `assign_sus_only_id`), `supabase/006_add_researcher_dashboard.sql` (the original dashboard migration), and `supabase/007_add_shared_background_and_identity.sql` (shared background, separately protected names, consent v2, and the enhanced dashboard RPC). For an existing project run 003, 004, 005, and 007 in order. If 006 was already run, leave it in place and run 007 afterwards.
